...
const PopularIndianSection = ({ sheading, fheading, secheading }) => {
  const [destinations, setIndianDestinations] = useState([]);

  useEffect(() => {
    const fetchIndianDest = async () => {
      try {
        const response = await axios.get("https://admin.climaxholidays.com/api/HolidayPackages/IndianDestinations");
        ...
        <div>
          <div className="section-title1">
            ...
            <h2 style={{ color: "#000000" }}>
              <span style={{ color: "#014e89" }}>{fheading}</span> {secheading}
            </h2>
            <div className="heading-ribbon">
              <svg
                className="svg-similar-reso"
                viewBox="0 0 370 18"
                fill="none"
              ...
              >
                ...
                <defs>
                  ...
                  >
                    ...
                    <stop offset={1} stopColor="#f73030" />
                  </linearGradient>
                </defs>
              </svg>
            </div>
          </div>
          <div style={{position:"relative"}} className="container-fluid overflow-hidden m-0 p-0">
                    ...
                    <Link to={`/${destination.slug}`}>
                      <div className="destination-single4">
                        <img
                          ...
                          className="img-lg-fluid wp-post-image"
                          alt={destination.name}
                          style={{ minHeight: "300px", objectFit: "cover", objectPosition: "center center" }}
                        />


                        <div className="desti-content">
                          <h4>
                            <Link to={`/${destination.slug}`}>{destination.name}</Link>
                          </h4>
                          {/* <span>1 Tours Place </span> */}
                        </div>
                      </div>
                    </Link>
                  </SwiperSlide>
            ...
            <div 
              className="custom-prevPopular"
              onClick={() => swiperRef.current?.slidePrev()}
            >
              <FaChevronLeft />
            </div>
            <div
              className="custom-nextPopular"
...
export default PopularIndianSection;
