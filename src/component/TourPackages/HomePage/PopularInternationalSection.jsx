...
const PopularInternationalSection = () => {
  const [destinations, setInternationalDestinations] = useState([]);

  useEffect(() => {
    const fetchInternationalDest = async () => {
      try {
        const response = await axios.get("https://admin.climaxholidays.com/api/HolidayPackages/InternationalDestinations");

        if (response.data && response.data.success === true && Array.isArray(response.data.data)) {
          setInternationalDestinations(response.data.data);
        }
      } catch (error) {
        console.error("Error fetching popular International Destinations:", error);
      }
    };
    fetchInternationalDest();
  }, []);
  return (
    <>
      {destinations.length > 0 && (
        <section className="destinations-one">
          <Container>
            <div className="section-title text-center">
              ...
              <h2 className="section-title__title mt-0" style={{ color: "#000000" }}>
                <span style={{ color: "#014e89" }}>Popular</span> International
                Destinations
              </h2>
           ...
           <Row>
            {destinations.map((destination, index) => (
              <Col key={index} md={index === 0 || index === 2 ? 3 : 6}>
                <div className="destinations-one__single">
                  <Link to={`/${destination.slug}`} className="destinations-one__img">
                    <img
          ...
          <div className="tourBtn3">
            <Link to="/international">Explore More</Link>
          </div>
        </section>
      )}
    </>
