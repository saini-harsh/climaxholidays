    ...
    const fetchTrending = async () => {
      try {
        const response = await axios.get(
          "https://admin.climaxholidays.com/api/HolidayPackages/InternationalDestinations"
        );
        ...
        <div>
          <Container>
            <h2 className="custom_heading ft-700 mt-20">
              <span className="text-b text-center">Top </span>International
              Destinations
            </h2>
              ...
              >
                {destinations.map((destination, index) => (
                  <SwiperSlide key={index}>
                    <Link
                      className="jckpt_box text-center tilt-card-box"
                      to={`/${destination.slug}`}
                    >
